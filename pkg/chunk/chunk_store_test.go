...
func TestChunkStore_Get(t *testing.T) {
	...
	assert.NoError(t, testutil.CollectAndCompare(indexLookupsPerQuery, strings.NewReader(observableMetadataIndexLookups), "cortex_chunk_store_index_lookups_per_query"))
...
func (m mockSeriesStoreSchema) GetCacheKeysAndLabelWriteEntries(from, through model.Time, userID string, metricName string, labels labels.Labels, chunkID string, excfg util.ExcludeLabels) ([]string, [][]IndexEntry, error) {
	return m.schema.GetCacheKeysAndLabelWriteEntries(from, through, userID, metricName, labels, chunkID, excfg)
}
...
func (m mockSeriesStoreSchema) GetSeriesDeleteEntries(from, through model.Time, userID string, metric labels.Labels, hasChunksForIntervalFunc hasChunksForIntervalFunc, excfg util.ExcludeLabels) ([]IndexEntry, error) {
	return m.schema.GetSeriesDeleteEntries(from, through, userID, metric, hasChunksForIntervalFunc, excfg)
}
